...
def get_topics_and_descriptions(political_position):
    response = basic_gpt.ask_mini_gpt(political_position, get_summary_instructions(), temperature=0)
...
def analyze_score(score, position, context, party, topic_criteria, recursive_depth=0):
    ...
    if ("detailed_answer" in score and "rating" in score and score["detailed_answer"] is not None
        ...
        if "wurde richtig bewertet" in response.lower():
            ...
            response = basic_gpt.ask_mini_gpt(p_pcs, get_reevaluate_rating_instructions())
            ...
            if "zu teilen richtig" in response.lower():
                    response = basic_gpt.ask_mini_gpt(p_pcs, get_adapted_opinion_for_better_context_instructions())
                    ...
                    response = basic_gpt.ask_mini_gpt(prompt, get_rag_prompt_template())
        ...
        response = basic_gpt.ask_mini_gpt(p_pcs, get_rating_from_response_instructions())
        if response.isdigit():
